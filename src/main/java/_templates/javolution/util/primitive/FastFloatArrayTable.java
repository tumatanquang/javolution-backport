...
/**
 * Implement <code>FastArrayTable</code> for the <code>float</code> primitive data type.
 * @since 5.7.1
 */
public strictfp class FastFloatArrayTable implements Cloneable, RandomAccess, Serializable {
	private transient float[] elementData;
	private int size;
	public FastFloatArrayTable() {
		this(10);
	}
	public FastFloatArrayTable(int initialCapacity) {
		if(initialCapacity < 0)
			throw new IllegalArgumentException("Illegal capacity: " + initialCapacity);
		elementData = new float[initialCapacity];
	}
	public FastFloatArrayTable(float[] src) {
		size = src.length;
	...
	public Object/*FastFloatArrayTable*/ clone() throws CloneNotSupportedException {
		/*@JVM-1.1+@
		if(true) {
			final FastFloatArrayTable c = (FastFloatArrayTable) super.clone();
	...
	public boolean addAll(FastFloatArrayTable c) {
		if(c.size == 0)
			return false;
	...
	public boolean addAll(int index, FastFloatArrayTable c) {
	...
	public boolean equals(Object o) {
		...
		if(o == null || getClass() != o.getClass())
			return false;
		final FastFloatArrayTable that = (FastFloatArrayTable) o;
		if(size != that.size)
			return false;
		for(int i = 0; i < size; ++i)
